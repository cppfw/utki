...
tst::set set("unicode", [](tst::suite& suite){
	...
	suite.add(
		"utf32_to_utf8_two_byte",
		[](){
			auto ret = utki::to_utf8(0x41a); // Capital cyrillic K
			tst::check_eq(ret[0], char(0xd0), [&](auto&o){o << "ret[0] = 0x" << std::hex << (unsigned(ret[0]) & 0xff);}, SL);
	...
	suite.add(
		"utf32_to_utf8_three_byte",
		[](){
			auto ret = utki::to_utf8(0xbf5); // ௵
			tst::check_eq(ret[0], char(0xe0), [&](auto&o){o << "ret[0] = 0x" << std::hex << (unsigned(ret[0]) & 0xff);}, SL);
			tst::check_eq(ret[1], char(0xaf), [&](auto&o){o << "ret[1] = 0x" << std::hex << (unsigned(ret[1]) & 0xff);}, SL);
	...
	suite.add(
		"utf32_to_utf8_four_byte",
		[](){
			auto ret = utki::to_utf8(0x26218); // 𦈘
			tst::check_eq(ret[0], char(0xf0), [&](auto&o){o << "ret[0] = 0x" << std::hex << (unsigned(ret[0]) & 0xff);}, SL);
			tst::check_eq(ret[1], char(0xa6), [&](auto&o){o << "ret[1] = 0x" << std::hex << (unsigned(ret[1]) & 0xff);}, SL);
	...
	suite.add(
		"utf32_to_utf8_five_byte",
		[](){
			auto ret = utki::to_utf8(0x3FFFFFF);
			tst::check_eq(ret[0], char(0xfb), [&](auto&o){o << "ret[0] = 0x" << std::hex << (unsigned(ret[0]) & 0xff);}, SL);
			tst::check_eq(ret[1], char(0xbf), [&](auto&o){o << "ret[1] = 0x" << std::hex << (unsigned(ret[1]) & 0xff);}, SL);
	...
	suite.add(
		"utf32_to_utf8_six_byte",
		[](){
			auto ret = utki::to_utf8(0x7FFFFFFF);
			tst::check_eq(ret[0], char(0xfd), [&](auto&o){o << "ret[0] = 0x" << std::hex << (unsigned(ret[0]) & 0xff);}, SL);
			tst::check_eq(ret[1], char(0xbf), [&](auto&o){o << "ret[1] = 0x" << std::hex << (unsigned(ret[1]) & 0xff);}, SL);
			tst::check_eq(ret[2], char(0xbf), SL);
			tst::check_eq(ret[3], char(0xbf), SL);
			tst::check_eq(ret[4], char(0xbf), SL);
